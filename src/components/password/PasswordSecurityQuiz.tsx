...
import { IconLock, IconCheck, IconX, IconShieldLock, IconKey, IconBrain } from '@tabler/icons-react'
...
export default function PasswordSecurityQuiz() {
  ...
  const loadQuizProgress = async () => {
    try {
      const response = await fetch('/api/quizzes/progress?type=password')
      const data = await response.json()
      
      const updatedQuizzes = userQuestions.map(quiz => ({
        ...quiz,
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4">
      ...
      >
        <div>
          ...
          <p className="text-gray-400 mt-2 text-lg">Test your understanding of password security best practices.</p>
        </div>
